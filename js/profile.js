...
(function () {
    ...
    if (!window.safeFetch) {
        window.safeFetch = async function (url, options = {}) {
            const res = await fetch(url, options); const text = await res.text(); let data = null;
            try { data = JSON.parse(text); } catch (_) {}
    ...
    if (!window.doLogin) window.doLogin = async function (username, password) { const data = await request('/login', { method:'POST', body:{ username, password } }); setUser({ username:data.username||username, token:data.token }); return data; };
    if (!window.doRegister) window.doRegister = async function (username, password) { const data = await request('/register', { method:'POST', body:{ username, password } }); setUser({ username:data.username||username, token:data.token }); return data; };
    ...
    async function render() {
        ...
        try { const data = await request('/users/me'); user = Object.assign({}, user, data.user || {}); if (data.token) user.token = data.token; setUser(user); }
        catch (e) { if (e.status === 401) return renderLogin(box); box.innerHTML = '<div style="color:#888;text-align:center;padding:2rem;">加载失败：' + escape(e.message) + '</div>'; return; }
    ...
    function renderLogin(box){
        ...
        const login=document.createElement('button');login.textContent='登录';login.onclick=async()=>{if(!username.field.value.trim()||!password.field.value)return alert('请填写完整');try{await window.doLogin(username.field.value.trim(),password.field.value);render();}catch(e){alert('登录失败：'+e.message);}};
    ...
    function renderEdit(box,user){
        box.innerHTML='';const wrap=document.createElement('div');wrap.onclick=e=>e.stopPropagation();const title=document.createElement('h3');title.textContent='编辑资料';title.style.cssText='color:#fff;margin-bottom:1rem;';wrap.appendChild(title);
        ...
        const actions=document.createElement('div');actions.style.cssText='display:flex;gap:10px;justify-content:flex-end;margin-top:1rem;';const cancel=document.createElement('button');cancel.textContent='取消';cancel.style.background='#555';cancel.onclick=()=>render();
        const save=document.createElement('button');save.textContent='保存';save.style.background='#d9534f';save.onclick=async()=>{const payload={avatar:avatar.field.value.trim()||null,school:school.field.value.trim()||null,honor_year:year.field.value.trim()||null,honor_rank:rank.field.value.trim()||null};if(newPassword.field.value){if(!oldPassword.field.value)return alert('请输入当前密码才能修改密码');payload.old_password=oldPassword.field.value;payload.password=newPassword.field.value;}try{const data=await request('/users/me',{method:'PUT',body:payload});setUser(Object.assign({},getUser(),data.user||{},data.token?{token:data.token}:{}));alert('资料更新成功！');render();}catch(e){alert('保存失败：'+e.message);}};
