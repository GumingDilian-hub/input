...
> **图16：苯丙氨酸代谢通路**
:::image center 015.png 图16：苯丙氨酸代谢通路 :::
...
如果你想知道更多（指300种），请移步细胞生物学板块一得病喽。

### 氨代谢
| 代谢物 | 说明 |
|---|---|
| NH3 | 硬骨鱼，两栖幼体，水形态肺鱼，海生无脊椎 |
| Asn | 植物 |
...
| 类型 | Km | 调节 | 说明 |
|---|---|---|---|
| 1血脑 | 低于血糖 | 不随血糖波动 | 唯心主义，脑子和心脏 |
| 2肝胰 | 高于血糖 | 检查出 GLC 浓度变化 | |
| 4肌脂 | 高 | 肌肉收缩和胰岛素调控 | |

#### 2. 已糖激酶和葡糖激酶
| 类型 | 调节 | 说明 |
|---|---|---|
| I脑 | G6P 的反馈抑制 | 唯心主义，脑子用前三个 |
| II脑 | 大 Km | |
| III脑 | 大 Km 和 Vm | |
| IV肝 | 胰岛素调节+Glc 诱导 | |

#### 3. 乳酸脱氢酶 LDH 同工酶
由 M（肌肉肝脏型）和 H（心脏型）构成的四聚体，依旧唯心主义，所以 LDH1 就是 H4，LDH5 就是 M5 啦。
#### 4. 肌酸激酶 CK 同工酶
由 M（肌肉型）和 B（脑子型）构成的二聚体，依旧唯心主义，所以 CK1 是 B2 型。*CK2MB 是心肌梗死指标（因为 MB 就是晨光，晨光很贵），CK3 是正常血液指标。*
#### 5. 丙酮酸激酶
- 负别构效应：ATP、Ala
- 正别构效应：F16BP、PKA
...
| 延伸 | Ser2/Ser5 磷酸化，维持延伸活性，招募剪切和修复因子 |
| RNA加工 | 动态磷酸化/去磷酸化，协调加帽、剪接、多聚A化 |
| 终止与修复 | 去磷酸化，触发终止并修复 DNA |
| 表观遗传 | 染色体调控与修饰酶结合 |
| 启动子清空 | TF II H 给 Ser+Pi，延伸的不可逆开关 |
...
| 嘌呤合成 | PRPP 合/氨酰转移酶 |
| 脂肪合成 | 脂酰 CoA 转移酶 |
| 尿素合成 | CPS I、Arg 代琥珀酸合酶 |
| 脂肪分解 | HSL（激素依赖性酶） |
| TCA 循环 | 宁异勿同，虎虎延平，一同平 |
| β 氧化 | CPT/CAT I、虎，二虎一能 |

#### 载脂蛋白
- Apo A：HDL、CM
- Apo B：VLDL、LDL(B100)、CM(B48)（只有这 2B 不能在脂滴之间流通）
- Apo C：CM、HDL、VLDL
- Apo D：HDL
- Apo E：CM、VLDL、HDL

1. 小肠 TG + ApoB48 + ApoA → CM；HDL 给 CM ApoC/E，CM 给 HDL Apo A。CM 成熟，ApoC 激活 LPL，水解 CM，最后 ApoA/C 解离变成 HDL，最后 HDL 被血管内皮 LDL 受体 LRP 识别胞吞。
2. 肝 TG + ApoB100 + ApoE → VLDL（←小肠黏膜也可以）。HDL 给 VLDL ApoC，VLDL 成熟，同样 ApoC 激活 LPL，变小。VLDL 把 ApoC 还给 HDL，给她胆固醇，HDL 给 VLDL 胆固醇酯。变成 IDL，一部分内吞，一部分为 HDL 捐出 ApoE 变成 LDL。
3. LDL 一部分被肝脏 LRP 吃掉，一部分被单核吞噬细胞系统（巨噬细胞和血管内皮清道夫受体 SR）。
...
| | 角蛋白 | 几丁质 |
|---|---|---|
| α型 | 富含 α 螺旋，富集 CESGAS，致密，毛角喙爪丝 | 反平行 β 折叠 |
| β型 | 富含 β 折叠，富于，蚕 | 平行 β 折叠，节肢动物刚毛、蚯蚓刚毛 |

#### 2. 最适氨基酸
- α 螺旋：meAKCIQH（我的 AK 氯气海）
- 平行 β 折叠：VILMFYW（口读放过这不了审，自个想去吧）
- 反平行 β 折叠：IWTAV（IWannaToAVal 我想要一双鞋）
- N 端规则稳定氨基酸：MV:GAST（假装这是一首 MV）
...
#### 4. DNA 超螺旋
DNA 希望 \(T = L = 60\)，当 \(L < T = 60\) 时如 \(L = 6\)，这就很难受了，因为 \(L\) 想等于他，但是 \(T\) 在断链的情况下永远不变，所以 \(L\) 只能单向奔赴，这时 \(W\) 是负数，所以说是负超螺旋。
...
- 氨基酸家族记忆法：“人走狗”三个字不要翻译（“不做人走狗”规则）
  - α-酮戊二酸：阿(α)珂(K)拍(P)钱(Q)而(ER)去(Q)
  - asp：冬(asp)天(DN)埋(M)汰(T)挨(I)冻(D)
  - PEP/赤藓糖：险(藓)些染上疯(WYF)病
  - 三磷酸甘油酸：三(3)个(G)人(不做人走狗)扛(Sc, 钒)走(不做人走狗)
