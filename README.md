...
如果你想让 AI 帮你把普通笔记转换成 **IWP 项目** 所需的特殊格式，请**完整复制下方代码块中的提示词**，发送给任意大语言模型（如 ChatGPT、Claude），并在后面附上你的原始笔记内容。

---

### 📋 复制以下提示词（点击代码块右上角即可复制）
...
请严格按照上述示例的输出格式，将用户下方提供的笔记内容转换为 IWP 格式。只输出转换后的 Markdown 源码，不要添加额外解释。
